...
#include "load_d2kpipi_MBRun2010data_part1.C"
#include "load_d2kpipi_MBRun2010data_part2.C"

void load_d2kpipi_MBRun2010data_part1(TChain*) ;
void load_d2kpipi_MBRun2010data_part2(TChain*) ;

Double_t GaussPol1 (Double_t * , Double_t *) ;
void MakeLegendTop(TLegend*, TLegendEntry*) ;
...
///////////////////////////////////////////////////////////////////////////////////////// 
{
   ...
   //  HISTOGRAMS FOR DIFFRACTIVE STUDIES

   TH1I *h_nHFPlus  = new TH1I("h_nHFPlus","Tower multiplicity in HF+",200,0,200) ;
   TH1I *h_nHFMinus = new TH1I("h_nHFMinus","Tower multiplicity in HF-",100,0,100) ;
   TH1F *h_HFPlusEnergyPF = new TH1F("h_HFPlusEnergyPF","PF Energy Sum in HF+",1000,0,2000) ;
   TH1F *h_HFMinusEnergyPF = new TH1F("h_HFMinusEnergyPF","PF Energy Sum in HF-",1000,0,1000) ;
   TH1F *h_etaMax = new TH1F("h_etaMax","PF #eta_{max}",200,-5.0,5.0) ;
   TH1F *h_etaMin = new TH1F("h_etaMin","PF #eta_{min}",100,-5.0,5.0) ;

   TH1F *h_mbest_nCalTowerEq0 = new TH1F("h_mbest_nCalTowerEq0","n_{HF+}= 0  OR  n_{HF-}= 0",NBINS,LOWER_MASS,HIGHER_MASS) ; 
   TH1F *h_mbest_PFEsumLessthan4 = new TH1F("h_mbest_PFEsumLessthan4","#Sigma E_{HF+}< 4 GeV  OR  #Sigma E_{HF-}< 4 GeV",NBINS,LOWER_MASS,HIGHER_MASS) ; 
   TH1F *h_fake_mbest_nCalTowerEq0 = new TH1F("h_fake_mbest_nCalTowerEq0","n_{HF+}= 0  OR  n_{HF-}= 0)",NBINS,LOWER_MASS,HIGHER_MASS) ; 
   TH1F *h_fake_mbest_PFEsumLessthan4 = new TH1F("h_fake_mbest_PFEsumLessthan4","#Sigma E_{HF+}< 4 GeV  OR  #Sigma E_{HF-}< 4 GeV",NBINS,LOWER_MASS,HIGHER_MASS) ; 
   TH1F *h_diff_mbest_nCalTowerEq0 = new TH1F("h_diff_mbest_nCalTowerEq0","n_{HF+}= 0  OR  n_{HF-}= 0",NBINS,LOWER_MASS,HIGHER_MASS) ; 
   TH1F *h_diff_mbest_PFEsumLessthan4 = new TH1F("h_diff_mbest_PFEsumLessthan4","#Sigma E_{HF+}< 4 GeV  OR  #Sigma E_{HF-}< 4 GeV",NBINS,LOWER_MASS,HIGHER_MASS) ; 

//   TH1F *h_refitMbest_nHFcut = new TH1F("h_refitMbest_nHFcut","Mass of D0 candidates (refitted tracks) for events with zero towers above threshold in one side of HF; K#pi Invariant Mass  (GeV/c^{2})",NBINS,LOWER_MASS,HIGHER_MASS) ; 
//   TH1F *h_fake_refitMbest_nHFcut = new TH1F("h_fake_refitMbest_nHFcut","Mass of fake D0 candidates (refitted tracks) for events with zero towers above threshold in one side of HF; K#pi Invariant Mass  (GeV/c^{2})",NBINS,LOWER_MASS,HIGHER_MASS) ; 
//   TH1F *h_diff_refitMbest_nHFcut = new TH1F("h_diff_refitMbest_nHFcut","Mass of D0 candidates (fake subtracted) for events with zero towers above threshold in one side of HF; K#pi Invariant Mass  (GeV/c^{2})",NBINS,LOWER_MASS,HIGHER_MASS) ; 
   ...
   double pT_1st_cut = 1.7 ;      //  std cut = 1.7 
   double pT_2nd_cut = 1.5 ;      //  std cut = 1.5 
   double pT_3rd_cut = 0.9 ;      //  std cut = 0.9 
...
*/   
   ...
   // load_d2kpipi_MBRun2010data_part1(fChain) ;
   load_d2kpipi_MBRun2010data_part2(fChain) ;
   // Init(fChain);
   if (!fChain) return 1;
...
//              trk1p>trk1p_cut  &&  trk2p>trk2p_cut  &&  trk3p>trk3p_cut  &&
      ...
      if(keep) {
         ...
         // Diffractive histograms
         // if(nHFPlus==0 || nHFMinus==0) h_refitMbest_nHFcut->Fill(refittedMassDplus) ; 
         if(nHFPlus==0 || nHFMinus==0) h_mbest_nCalTowerEq0->Fill(refittedMassDplus) ;
         if(HFPlusEnergyFromPF<4.0 || HFMinusEnergyFromPF<4.0) h_mbest_PFEsumLessthan4->Fill(refittedMassDplus) ;
      ...
      // Diffractive distributions
      h_nHFPlus->Fill(nHFPlus) ;
      h_nHFMinus->Fill(nHFMinus) ;
      h_HFPlusEnergyPF->Fill(HFPlusEnergyFromPF) ;
      h_HFMinusEnergyPF->Fill(HFMinusEnergyFromPF) ;
      h_etaMax->Fill(etaMaxFromPF) ;
      h_etaMin->Fill(etaMinFromPF) ;
   }
...
//              trk1p>trk1p_cut  &&  trk2p>trk2p_cut  &&  trk3p>trk3p_cut  &&
      ...
      if(keep) {
         ...
         // Diffractive histograms
         //if(nHFPlus==0 || nHFMinus==0) h_fake_refitMbest_nHFcut->Fill(refittedMassDplus) ; 
         if(nHFPlus==0 || nHFMinus==0) h_fake_mbest_nCalTowerEq0->Fill(refittedMassDplus) ;
         if(HFPlusEnergyFromPF<4.0 || HFMinusEnergyFromPF<4.0) h_fake_mbest_PFEsumLessthan4->Fill(refittedMassDplus) ;
      }
      if( trk1pT>trk1pT_cut && trk2pT>trk2pT_cut && trk3pT>trk3pT_cut &&
           trk1p>trk1p_cut  &&  trk2p>trk2p_cut  &&  trk3p>trk3p_cut  &&
...
/*
   ...
   //  Create Canvas
   TCanvas *c9 = new TCanvas("c9","Mass distribution of D+ -> Kpipi candidates",50,150,600,800);
   c9->Divide(1,2) ;
   c9->cd(1) ;
...
*/
   ...
   /////////////////////////////////////////////////////
   //                                                 //
   //   All candidates passing basic selection cuts   //
   //                                                 //
   /////////////////////////////////////////////////////
   ...
   //  Create Canvas
   TCanvas *c1 = new TCanvas("c1","Refitted mass distribution of D+ -> Kpipi candidates",700,150,600,800);
   c1->Divide(1,2) ;
   c1->cd(1) ;
   ...
   g1->SetParameter(0,max2);
   g1->SetParameter(1,nominal_mass);
   g1->SetParameter(2,0.010);
   // g1->SetParNames("Nevents","Mean","Sigma","Const");
   g1->SetParNames("Nevents","Mean","Sigma","Const","Slope");  //  Only with GaussPol1
   
   g1->SetParLimits(0,0,10*max2);
   g1->SetParLimits(1,nominal_mass-0.005,nominal_mass+0.005);
   g1->SetParLimits(2,0.005,0.020);

   h_refitMassDplus->Fit("g1","B","SAME",Mmin,Mmax);
   ...
   gdif1->SetParameter(0,difmax2);
   gdif1->SetParameter(1,nominal_mass);
   gdif1->SetParameter(2,0.010);
   // gdif1->SetParNames("Nevents","Mean","Sigma","Const");
   gdif1->SetParNames("Nevents","Mean","Sigma","Const","Slope");  //  Only with GaussPol1
   // gdif1->FixParameter(4,0.);
   
   gdif1->SetParLimits(0,0,10*difmax2);
   gdif1->SetParLimits(1,nominal_mass-0.005,nominal_mass+0.005);
   gdif1->SetParLimits(2,0.005,0.020);

   h_diff_refitMassDplus->Fit("gdif1","B","SAME",Mmin,Mmax);
   // h_diff_refitMassDplus->Fit("gdif1","BWW","SAME",Mmin,Mmax);
   ...
   //  Create Canvas
   TCanvas *c2 = new TCanvas("c2","Mass window cut study",700,150,1400,700);
   c2->Divide(4,2) ;
...
*/
   ...
   //  Create Canvas
   TCanvas *c14 = new TCanvas("c14","Invariant mass of Kpi pairs from D+ -> Kpipi candidates",100,150,600,800);
   ...
   //////////////////////////////////////////////////////////
   //                                                      //
   //         Inclusive Diffractive Distributions          //
   //                                                      //
   //////////////////////////////////////////////////////////
   
   //  Create Canvas
   TCanvas *c3 = new TCanvas("c3","Inclusive Diffractive Distributions",50,100,1050,700);
   c3->Divide(3,2) ;

   c3->cd(1) ;
   h_nHFPlus->Draw("e1") ;
   h_nHFPlus->SetLineColor(4);

   c3->cd(4) ;
   h_nHFMinus->Draw("e1") ;
   h_nHFMinus->SetLineColor(4);

   c3->cd(2) ;
   h_HFPlusEnergyPF->Draw("e1") ;
   h_HFPlusEnergyPF->SetLineColor(4);
   h_HFPlusEnergyPF->GetXaxis()->SetTitle("GeV");

   c3->cd(5) ;
   h_HFMinusEnergyPF->Draw("e1") ;
   h_HFMinusEnergyPF->SetLineColor(4);
   h_HFMinusEnergyPF->GetXaxis()->SetTitle("GeV");

   c3->cd(3) ;
   h_etaMax->Draw("e1") ;
   h_etaMax->SetLineColor(4);

   c3->cd(6) ;
   h_etaMin->Draw("e1") ;
   h_etaMin->SetLineColor(4);

   
   //////////////////////////////////////////////////////////
   //                                                      //
   //  Refitted mass distribution of D0 -> Kpi candidates  //
   //                with diffractive cuts                 //
   //                                                      //
   //////////////////////////////////////////////////////////
   
   h_diff_mbest_PFEsumLessthan4->Sumw2() ;
   h_diff_mbest_PFEsumLessthan4->Add(h_mbest_PFEsumLessthan4,h_fake_mbest_PFEsumLessthan4,1,-1) ;
   
   h_diff_mbest_nCalTowerEq0->Sumw2() ;
   h_diff_mbest_nCalTowerEq0->Add(h_mbest_nCalTowerEq0,h_fake_mbest_nCalTowerEq0,1,-1) ;
   
   //  Create Canvas
   TCanvas *c4 = new TCanvas("c4","Refitted mass distribution of D+ -> Kpipi candidates with diffractive cuts",200,150,1400,700);
   ...
   h_refitMassDplus->Draw("e"); 
   h_refitMassDplus->GetXaxis()->CenterTitle();
   h_fake_refitMassDplus->Draw("same"); 
   
   MakeLegendTop(leg,entry);
   c4_1->Modified();
   ...
   // h_mbest_PFEsumLessthan4->SetLineColor(4);       // h_mbest_PFEsumLessthan4->SetLineStyle(1); 
   h_mbest_PFEsumLessthan4->SetLineWidth(2); 
   h_mbest_PFEsumLessthan4->Draw("e"); 
   h_mbest_PFEsumLessthan4->GetXaxis()->CenterTitle();
   h_mbest_PFEsumLessthan4->GetXaxis()->SetTitle("K#pi Invariant Mass  (GeV/c^{2})");
   h_fake_mbest_PFEsumLessthan4->SetLineColor(2);  // h_fake_mbest_PFEsumLessthan4->SetLineStyle(1); 
   h_fake_mbest_PFEsumLessthan4->SetFillColor(2);
   h_fake_mbest_PFEsumLessthan4->SetFillStyle(3001);
   h_fake_mbest_PFEsumLessthan4->Draw("same"); 

   // Fit histogram

   TF1 *gpfe = new TF1("gpfe",GaussPol1,Mmin,Mmax,5);
   
   Double_t maxpfe   = h_mbest_PFEsumLessthan4->GetMaximum();
   gpfe->SetParameter(0,maxpfe);
   gpfe->SetParameter(1,1.865);
   gpfe->SetParameter(2,0.010);
   gpfe->SetParNames("Nevents","Mean","Sigma","Const","Slope");  //  Only with GaussPol1
   gpfe->SetParLimits(0,0,10*maxpfe);
   gpfe->SetParLimits(1,1.855,1.875);
   gpfe->SetParLimits(2,0.012,0.018);

   gpfe->SetLineColor(4);
   gpfe->SetLineWidth(2);
   
   // h_mbest_PFEsumLessthan4->Fit("gpfe","B","SAME",Mmin,Mmax);
   h_mbest_PFEsumLessthan4->Fit("gpfe","BWW","SAME",Mmin,Mmax);
   ...
   // h_mbest_nCalTowerEq0->SetLineColor(4);       // h_mbest_nCalTowerEq0->SetLineStyle(1); 
   h_mbest_nCalTowerEq0->SetLineWidth(2);
   h_mbest_nCalTowerEq0->Draw("e"); 
   h_mbest_nCalTowerEq0->GetXaxis()->CenterTitle();
   h_mbest_nCalTowerEq0->GetXaxis()->SetTitle("K#pi Invariant Mass  (GeV/c^{2})");
   h_fake_mbest_nCalTowerEq0->SetLineColor(2);  // h_fake_mbest_nCalTowerEq0->SetLineStyle(1); 
   h_fake_mbest_nCalTowerEq0->SetFillColor(2);
   h_fake_mbest_nCalTowerEq0->SetFillStyle(3001);
   h_fake_mbest_nCalTowerEq0->Draw("same"); 
   
   // Fit histogram

   TF1 *gnhf = new TF1("gnhf",GaussPol1,Mmin,Mmax,5);
   
   Double_t maxnhf   = h_mbest_nCalTowerEq0->GetMaximum();
   gnhf->SetParameter(0,maxnhf);
   gnhf->SetParameter(1,1.865);
   gnhf->SetParameter(2,0.010);
   gnhf->SetParNames("Nevents","Mean","Sigma","Const","Slope");  //  Only with GaussPol1
   gnhf->SetParLimits(0,0,10*maxnhf);
   gnhf->SetParLimits(1,1.855,1.875);
   gnhf->SetParLimits(2,0.012,0.018);
   
   gnhf->SetLineColor(4);
   gnhf->SetLineWidth(2);

   // h_mbest_nCalTowerEq0->Fit("gnhf","B","SAME",Mmin,Mmax);
   h_mbest_nCalTowerEq0->Fit("gnhf","BWW","SAME",Mmin,Mmax);
   ...
   h_diff_refitMassDplus->SetLineWidth(2);
   h_diff_refitMassDplus->Draw("e");
   h_diff_refitMassDplus->GetXaxis()->CenterTitle();
   
   MakeLegendBottom(leg,entry);
   c4_4->Modified();
   ...
   // h_diff_mbest_PFEsumLessthan4->SetLineColor(46);
   h_diff_mbest_PFEsumLessthan4->SetLineWidth(2);
   h_diff_mbest_PFEsumLessthan4->Draw("e");
   h_diff_mbest_PFEsumLessthan4->GetXaxis()->CenterTitle();
   h_diff_mbest_PFEsumLessthan4->GetXaxis()->SetTitle("K#pi Invariant Mass  (GeV/c^{2})");
   
   // Fit histogram

   TF1 *gdifpfe = new TF1("gdifpfe",GaussPol1,Mmin,Mmax,5);
   
   Double_t diffmaxpfe   = h_diff_mbest_PFEsumLessthan4->GetMaximum();
   gdifpfe->SetParameter(0,diffmaxpfe);
   // gdifpfe->SetParameter(1,1.865);
   // gdifpfe->SetParameter(2,0.010);
   gdifpfe->SetParNames("Nevents","Mean","Sigma","Const","Slope");  //  Only with GaussPol1
   // ---   Set limits to parameters   ---
   gdifpfe->SetParLimits(0,0,10*diffmaxpfe);
   // gdifpfe->SetParLimits(1,1.850,1.880);
   // gdifpfe->SetParLimits(2,0.002,0.020);
   // ---   Fix parameters   ---
   gdifpfe->FixParameter(1,1.865);
   gdifpfe->FixParameter(2,0.015);

   gdifpfe->SetLineColor(4);
   gdifpfe->SetLineWidth(2);
   
   // h_diff_mbest_PFEsumLessthan4->Fit("gdifpfe","B","SAME",Mmin,Mmax);
   h_diff_mbest_PFEsumLessthan4->Fit("gdifpfe","BWW","SAME",Mmin,Mmax);
   ...
   // h_diff_mbest_nCalTowerEq0->SetLineColor(46);
   h_diff_mbest_nCalTowerEq0->SetLineWidth(2);
   h_diff_mbest_nCalTowerEq0->Draw("e");
   h_diff_mbest_nCalTowerEq0->GetXaxis()->CenterTitle();
   h_diff_mbest_nCalTowerEq0->GetXaxis()->SetTitle("K#pi Invariant Mass  (GeV/c^{2})");
   
   // Fit histogram

   TF1 *gdifnhf = new TF1("gdifnhf",GaussPol1,Mmin,Mmax,5);
   
   Double_t diffmaxnhf   = h_diff_mbest_nCalTowerEq0->GetMaximum();
   gdifnhf->SetParameter(0,diffmaxnhf);
   // gdifnhf->SetParameter(1,1.865);
   // gdifnhf->SetParameter(2,0.010);
   gdifnhf->SetParNames("Nevents","Mean","Sigma","Const","Slope");  //  Only with GaussPol1
   // ---   Set limits to parameters   ---
   gdifnhf->SetParLimits(0,0,10*diffmaxnhf);
   // gdifnhf->SetParLimits(1,1.850,1.880);
   // gdifnhf->SetParLimits(2,0.002,0.020);
   // ---   Fix parameters   ---
   gdifnhf->FixParameter(1,1.865);
   gdifnhf->FixParameter(2,0.015);

   gdifnhf->SetLineColor(4);
   gdifnhf->SetLineWidth(2);
   
   // h_diff_mbest_nCalTowerEq0->Fit("gdifnhf","B","SAME",Mmin,Mmax);
   h_diff_mbest_nCalTowerEq0->Fit("gdifnhf","BWW","SAME",Mmin,Mmax);
...
Double_t GaussPol1 (Double_t *v , Double_t *par)
{
   ...
   if (par[2] != 0) { 
      arg = (v[0] - par[1])/par[2] ;
      normGauss = sqrt(2*3.14159263) * par[2] ;
   }
//   Double_t fitval = par[0]*TMath::Exp(-0.5*arg*arg);
   Double_t gauss = (par[0]*binwidth) * ( exp(-0.5*arg*arg) / normGauss ) ;
...
void MakeLegendTop(TLegend *leg , TLegendEntry *entry)
{
   leg = new TLegend(0.14,0.60,0.42,0.86,NULL,"brNDC");
   leg->SetTextFont(62);
   leg->SetLineColor(1);
   leg->SetLineStyle(1);
   leg->SetLineWidth(1);
   ...
   entry=leg->AddEntry("NULL","Run2010A Data","h");
   ...
   entry=leg->AddEntry("h_refitMassDplus","opposite sign","lpf");
   ...
   entry->SetMarkerSize(1);
   entry=leg->AddEntry("h_fake_refitMassDplus","same sign","lpf");
...
void MakeLegendBottom(TLegend *leg , TLegendEntry *entry)
{
   leg = new TLegend(0.15,0.70,0.40,0.85,NULL,"brNDC");
   leg->SetTextFont(62);
   ...
   entry->SetMarkerSize(1);
   entry->SetTextFont(62);
   entry=leg->AddEntry("h_refitMassDplus","  D^{+} #rightarrow K#pi#pi","lpf");
   entry->SetFillStyle(1001);
   entry->SetLineColor(1);
   entry->SetLineStyle(1);
   entry->SetLineWidth(2);
